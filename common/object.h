...
*/
#define GUNGNIR_ENABLE_OBJECT_TRACKING 0
...
*/
#define GUNGNIR_ENABLE_REF_TRACKING 0

#if GUNGNIR_ENABLE_REF_TRACKING
#if !GUNGNIR_ENABLE_OBJECT_TRACKING
#error "GUNGNIR_ENABLE_REF_TRACKING requires GUNGNIR_ENABLE_OBJECT_TRACKING"
#endif
#include <map>
#include <mutex>
#endif
...
public:
   ...
   /// Note: This reports the actual class name if GUNGNIR_OBJECT() is used.
...
template<typename T>
struct fmt::formatter<gungnir::ref<T>> : formatter<const void*>
{
   template<typename FormatContext>
   auto format(const gungnir::ref<T>& ref, FormatContext& ctx) const
   {
...
template<typename T>
struct fmt::formatter<gungnir::BreakableReference<T>> : formatter<const void*>
{
   template<typename FormatContext>
   auto format(const gungnir::BreakableReference<T>& ref, FormatContext& ctx) const
   {
       return formatter<const void*>::format(ref.get(), ctx);
   }
};
...
{
template<typename T>
void swap(::gungnir::ref<T>& x, ::gungnir::ref<T>& y) noexcept
{
   return x.swap(y);
}

template<typename T>
struct hash<::gungnir::ref<T>>
{
   constexpr size_t operator()(const ::gungnir::ref<T>& r) const { return std::hash<T*>()(r.get()); }
};
